...
- Brittleness probes are scored separately from correctness mutation score. Surviving behavior-preserving brittleness probes are acceptable; killed brittleness probes should steer agents toward loosening implementation-coupled assertions rather than adding behavior assertions.
- `veritas score` reads `.veritas/report.json` and summarizes confidence from correctness mutation score, brittleness probe survival, findings, assertion candidates, corpus entries/replay, replay cases, baseline deltas, and budget health.
...
- TypeScript/JavaScript supports project detection from package/config/source roots, Tree-sitter target discovery for `.ts`, `.tsx`, `.js`, `.jsx`, `.mjs`, and `.cjs`, symbol graphs, package-manager-aware Bun/npm/pnpm/yarn test commands, executable Bun property checks for supported exported free functions, optional Bun lcov coverage, Tree-sitter-backed mutation checks for comparisons/equality/boolean/optional/nullish/async/spread/env/http/default/string-normalization operators, batched primitive free-function replay, and skipped Bun regression scaffolds from `promote-regression`. If the selected JS runtime is not installed, `verify` records a skipped command instead of failing discovery.
