...
Core also exposes a plugin-generic isolated mutation root helper. A language plugin can copy the target project into a temporary root, apply one mutant there, run the owning test command, and let cleanup happen on drop. Go and Rust mutation use this path when `[mutation].workers > 1`; `workers = 1` keeps the guarded serial source-rewrite fallback for local troubleshooting. Mutation metrics record requested workers, effective workers, setup milliseconds, copy milliseconds, excluded-path counts/samples, exclusion patterns, per-worker copy records, timed-out mutants, skipped mutants, scratch roots on isolation failures, and isolation failures so CI can separate performance behavior from mutation quality.
