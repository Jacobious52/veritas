...
The `brittleness` domain is intentionally scored separately from correctness mutation score. Enable it when you want to find tests coupled to behavior-preserving implementation details such as incidental ordering, formatting, logs, or non-contract normalization. A survived brittleness probe is usually fine; a killed brittleness probe asks the agent to loosen the test into a behavior assertion, not to add another assertion that kills the equivalent-style mutant. Disable noisy brittleness probes with `disabled_domains = ["brittleness"]`, or keep the domain and filter a specific operator/path with `disabled_operators`, `exclude_paths`, or `veritas:skip-mutation` when the detail is truly contractual.
