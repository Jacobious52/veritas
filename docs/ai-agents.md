...
7. Run `veritas next --explain` for the single highest-priority finding or evolution candidate, including estimated confidence impact, proof commands, and done-when criteria.
8. Run `veritas repair-prompt` for the concise AI repair queue, patch plan, mutation trend summary, proof commands, and done-when criteria. In GitHub Actions, use `veritas repair-prompt --github-step-summary`.
9. Run `veritas agent-instructions --agent codex` to write a repo-local workflow guide for future agent sessions.
10. Run `veritas review-packet` when you want a blind, deterministic senior-review packet grounded in mechanical Veritas evidence.
